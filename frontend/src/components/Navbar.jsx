...
const Navbar = () => {
  // here we will get the user data from the redux store and display the credits in the navbar - means how many times the user has used the ai to generate notes, we will display that in the navbar as credits remaining or something like that
  const { userData } = useSelector((state) => state.user); // we will get the credits from the userData and display it in the navbar
          ...
          >
            ...
            <AnimatePresence>
            {
                showCredits && 
                    ...
                    className="absolute -right-12.5 mt-4 w-64 rounded-2xl bg-black/90 backdrop-blur-xl border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.7)] p-4 text-white"
                    >
                        ...
                        className="w-full py-2 rounded-lg bg-linear-to-br from-white to-gray-200 text-black font-semibold">
                            Buy More Credits
                        </button>
                    </motion.div>
            }
...
function MenuItem({ onClick, text, red }) {
  return (
    <div
     onClick={onClick}
     className={`w-full text-left px-5 py-3 text-sm transition-colors cursor-pointer rounded-lg
      ${
        red
